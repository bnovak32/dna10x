...
def contig_address(address, chrs, bamout, bcset, insert_size, minscore):
    outfiles = [f"{address}.{ch}.address.txt.gz" for ch in chrs]
    print(bamout)
    print(len(outfiles))
    ...
    with AlignmentFile(bamout, 'rb') as f:
        ...
        for read in f:
            ...
            isize = read.tlen
            rlen = read.rlen
            if read.is_read1 and score > 0 and isize != 0 and score / rlen > minscore and insert_size > abs(isize) > 6:
                ...
                ch = f.get_reference_name(read.reference_id)
                if isize > 0:
                    ...
                    p2 = p1 - isize - 5
                newline = f"{readid}\t{cbc}\t{qcbc}\t{ch}\t{str(p1)}\t{str(p2)}\t{str(rlen)}\t{str(score)}\t{str(isize)}"
                ...
                if readid in rdict:
                    ...
                    if score > 0 and score / rlen > minscore:
                        ...
                        cbc = nl.split()[1]
                        ch = nl.split()[3]
                        newline = f"{nl}\t{str(rlen)}\t{str(score)}\n"
